...
#%% main    
landmark_path = '/data/msra_lfw/lfw/lfw_landmark.txt' #标定好的人脸关键点 左右眼
save_cropped_dir = '/data/msra_lfw/lfw/lfw_cropped/'
img_size = 224 #裁剪后的大小

if not os.path.exists(save_cropped_dir):
    os.mkdir(save_cropped_dir)
f = open('/data/msra_lfw/lfw/lfw_landmark_cropped.txt','w') # saved dir
label_landmark = open(landmark_path).readlines() #old label and landmark file
num = 1
length = len(label_landmark)
for line in label_landmark:
    temp = line.split(' ')
    img_path, landmark = temp[0], temp[1:]
    leftx, lefty, rightx, righty = float(landmark[0]), float(landmark[1]), float(landmark[2]), float(landmark[3])
    im = '/data/msra_lfw/lfw/' + img_path
    img_path_split = img_path.split('/')
    image = Image.open(im)
    if not os.path.exists(save_cropped_dir + img_path_split[1]):
        os.mkdir(save_cropped_dir + img_path_split[1])
    CropFace(image, eye_left=(leftx,lefty), eye_right=(rightx,righty), offset_pct=(0.3,0.3),
             dest_sz=(img_size,img_size)).save(save_cropped_dir + img_path_split[1]+'/'+img_path_split[2])
    
    f.write(save_cropped_dir + img_path_split[1]+'/'+img_path_split[2])
    f.write('\n')
    if num % 500 == 0:
